...
class AuthProvider with ChangeNotifier {
  ...
  Future<void> signUp(String name, String email, String password) async {
    await _callAuthMethod(() async {
      ...
      // Update the profile with the provided name immediately after sign up
      await updateUserProfile(newName: name, newEmail: email);
    });
  }
  ...
  Future<void> updateUserBudget(double newBudget) async {
    ...
    try {
      // Update Firestore
      await firestoreService.updateUserData(_user!.uid, {'budget': newBudget});

      // Update local profile
      _userProfile = _userProfile?.copyWith(budget: newBudget);
    } catch (e) {
      _errorMessage = e.toString();
      rethrow;
    } finally {
